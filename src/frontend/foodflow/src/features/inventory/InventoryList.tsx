...
export default function InventoryList() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getItems(restaurantid).then((response) => {
    ...
    <Card>
        ...
        <div className="d-flex justify-content-between align-items-center mb-3">
          <div>
            ...
            <Card.Title className="mb-0">
              {branchInventories[0]?.branchName ?? "Branch Inventory"}
            </Card.Title>
          </div>
          <div className="d-flex gap-2">
            <Link to={`/restaurants/${restaurantid}`} className="btn btn-outline-secondary">
              Back
            </Link>
            <Button variant="primary" onClick={() => setShowAddModal(true)} disabled={availableItems.length === 0}>
              Add Item
            </Button>
          </div>
        </div>
              ...
              {branchInventories.map((inventory) => (
                <tr key={inventory.inventoryId}>
                  ...
                  <td className="text-end">
                    <span className={inventory.quantity === 0 ? "text-danger fw-semibold" : "fw-semibold"}>
                      {inventory.quantity}
                    </span>
                  ...
                  <td>
                    <div className="d-flex gap-1">
                      <Button size="sm" variant="outline-primary" onClick={() => openStockModal(inventory, "add")}>
                        Add Stock
                      </Button>
                      <Button
                        size="sm"
      ...
      <Modal show={showStockModal} onHide={() => setShowStockModal(false)}>
        <Form onSubmit={handleStockUpdate}>
          <Modal.Header closeButton>
            <Modal.Title>{stockMode === "add" ? "Add Stock" : "Remove Stock"}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <div className="mb-3">
              <div className="fw-semibold">{selectedInventory?.itemName}</div>
              <div className="text-muted">Current stock: {selectedInventory?.quantity ?? 0}</div>
            </div>
            <Form.Group>
          ...
          <Modal.Footer>
            ...
            <Button variant={stockMode === "add" ? "primary" : "danger"} type="submit">
